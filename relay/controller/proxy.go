...
// RelayMessagesHelper is a helper function to relay Claude messages API with pass-through response
func RelayMessagesHelper(c *gin.Context) *relaymodel.ErrorWithStatusCode {
	ctx := c.Request.Context()
	meta := meta.GetByContext(c)
	// Override API type to use ClaudeMessages adaptor
	meta.APIType = apitype.ClaudeMessages
